...
| **Skills**    | `skills/<name>/SKILL.md` | A capability that triggers on intent        | a trigger phrase, `/<name>`, or Cursor auto-invoke |
...
1. Copy these files into your project's `.claude/` directory — this repo *is* that directory's contents. Skip `.cursor/` (it exists only so this repo, opened directly, exposes the same skills). Keep `settings.json` as valid JSON. Cursor then loads the skills from `.claude/skills/`.
...
```
.claude/                 # this repo's contents, copied into a project
├── README.md          this file
...
├── skills/<n>/SKILL.md  capabilities that trigger on intent (write-test, api-endpoint, …)
...
.cursor/skills/<n>     # symlinks to ../../skills/<n> — Cursor discovery when this repo is the workspace
```
...
- **Skills** — a directory `skills/<name>/` with exactly `SKILL.md`. The directory name matches the frontmatter `name` (lowercase-hyphenated). Frontmatter is Cursor's Agent Skills shape: `name` plus a third-person `description` that says what it does and when to use it (include the trigger phrases). Leave `disable-model-invocation` unset so the skill fires on intent. Do not set `tools:` — Cursor does not use it. The last phase is always `Report`. Paths inside a skill resolve a pack root: `.claude` when `.claude/tools/facts.sh` exists, otherwise the repository root.
